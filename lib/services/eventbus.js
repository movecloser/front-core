...
let Eventbus = class Eventbus {
    ...
    handle(name, callback) {
        return this._stream$.pipe(operators_1.filter((event) => event.name === name)).subscribe(/* istanbul ignore next */ (event) => {
    ...
    handleOnce(name, callback) {
        return this._stream$.pipe(operators_1.first((event) => event.name === name)).subscribe(/* istanbul ignore next */ (event) => {
            setTimeout(() => callback(event), 1);
        });
    }
};
