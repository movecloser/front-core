...
let Validation = class Validation {
    ...
    onClear(form, callback) {
        return this._stream$.pipe(operators_1.filter(event => event.form === form || event.type === validation_1.ValidationEventType.Clear)).subscribe(() => callback);
    }
    /**
    ...
    onErrors(form, field, callback) {
        return this._stream$.pipe(operators_1.filter(event => event.form === form &&
            event.type === validation_1.ValidationEventType.Error), operators_1.filter(event => typeof event.errors !== 'undefined' && field in event.errors), operators_1.map(event => {
